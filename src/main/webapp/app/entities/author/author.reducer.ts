...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_AUTHOR: 'author/PARTIAL_UPDATE_AUTHOR',
...
export default (state: AuthorState = initialState, action): AuthorState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_AUTHOR):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_AUTHOR):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_AUTHOR):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IAuthor> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_AUTHOR,
